...
open class AlamofireRequestBuilder<T>: RequestBuilder<T> {
    ...
    /**
     May be overridden by a subclass if the full `DataResponse<T>` returned by Alamofire need to be accessed.
     Needed because by default only a simpler `Response<T>`is returned where – for example – the original `URLRequest` is omitted
     Note: the purpose of overriding this shouldn't be to process the response but to execute any supplementary actions (e.g. logging)
    */
    open func dataResponse<T>(response: DataResponse<T>) {}
    ...
    fileprivate func processRequest(request: DataRequest, _ managerId: String, _ completion: @escaping (_ response: Response<T>?, _ error: Error?) -> Void) {
        ...
        case is String.Type:
            validatedRequest.responseString(completionHandler: { (stringResponse) in
                cleanupRequest()
                self.dataResponse(response: stringResponse)
        ...
        case is URL.Type:
            validatedRequest.responseData(completionHandler: { (dataResponse) in
                cleanupRequest()
                self.dataResponse(response: dataResponse)
        ...
        case is Void.Type:
            validatedRequest.responseData(completionHandler: { (voidResponse) in
                cleanupRequest()
                self.dataResponse(response: voidResponse)
